...
A CTMC with states \\( 0, 1, \ldots \\) for which \\( q_{ij} = 0 \\) whenever \\( \left|i - j\right| > 1 \\) is called a *birth and death process*. Let \\( \lambda_i = q_{i,i+1} \\), \\( \mu_i = q_{i,i-1} \\) be the birth rates and death rates, respectively. Then \\( v_i = \lambda_i + \mu_i \\), \\( P_{i,i+1} = \frac{\lambda_i}{\lambda_i + \mu_i} \\).
...
A stochastic process \\( \\{Z_n, n = 0, 1, \ldots\\} \\) is a martingale if \\( \mathbb{E}[\left|Z_n\right|] < \infty \\) and \\( \mathbb{E}[Z_{n+1} \mid Z_1, \ldots, Z_n] = Z_n \\) for all \\( n \\).
...
If \\( X_i \\) are i.i.d. with \\( E[\left|X\right|] < \infty \\) and \\( N \\) is a stopping time for \\( X_i \\) with \\( E[N] < \infty \\), then:
